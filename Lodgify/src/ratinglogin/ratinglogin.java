...
public class ratinglogin {
	...
	private void initialize() {
		...
		btnRate.setBackground(Color.WHITE);
		btnRate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				int yes = 0;
				int i = 0;
				int j = 0;
				int[] arru = new int[100];
			    ...
			    while(rs.next())
			    {
			    	...
			    	for (j=0;j<100;j++)
			    	{
			    		if (Integer.parseInt(username)==arru[j])
			    		{
			    			for(int k=0;k<100;k++)
			    			if(Integer.parseInt(password)== arrp[k])
			    			{
			    				yes = 1;
			    			}
			    			else 
